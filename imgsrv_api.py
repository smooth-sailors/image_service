...
- Generates original / medium / thumbnail
...
Replaces the original text-file polling pipeline in the assignment version.
"""
...
# Resized targets (need image sizes for other apps using image service)
...
def get_project_image(
    project_id: str,
    image_id: str,
    size: Literal["original", "medium", "thumb"] = "original",
) -> FileResponse:
    with _meta_lock:
        meta = _read_meta(project_id)
...
def delete_project_image(project_id: str, image_id: str) -> Dict[str, Any]:
    with _meta_lock:
        ...
        for k in ("original", "medium", "thumb"):
    ...
    return {"ok": True, "project_id": project_id, "deleted_image_id": image_id, "new_primary": meta.get("primary_image_id")}
